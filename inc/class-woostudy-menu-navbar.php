...
class Woostudy_Menu_Navbar extends Walker_Nav_menu{
	...
	public function start_lvl( &$output, $depth = 0, $args = null ) {
		...
		// Default class.ДОБАВЛЯЕМ СВОИ КЛАССЫ ВЛОЖЕННОМУ МЕНЮ
	...
	public function end_lvl( &$output, $depth = 0, $args = null ) {
        ...
        //ЗАМЕНЯЕМ UL НА DIV И ДОБАВЛЯЕМ ЕЩЁ ЗАКРЫВАЮЩИЙ DIV КОТОРЫЙ ПИСАЛИ ВЫШЕ
		// $output .= "$indent</ul>{$n}";
...
// Добавляем активный класс на странице на которой находимся
		$active_class = $menu_item->current ? ' active' : '';
//Добавлеям активный класс вложенному меню
		$active_parent_class = $menu_item->current_item_parent ? ' active' : '';

       
// МОЙ КОД ДОБАВЛЯЕМ ПЕРВОЙ ССЫЛКЕ
//$depth-уровень вложенности. $atts-сами ссылки в меню , сейчас это тег а
//присваиваем ссылкам класс nav-link dropdown-toggle и дата атрибут data-toggle
        ...
        //Мой код ставим <i class="fa fa-angle-right float-right mt-1"></i> перед закрывающей ссылкой
        if($this->has_children){
